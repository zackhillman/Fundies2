...
interface ILoFeature {
  ...
  int totalCapacityAcc(ILoFeature acc);

  boolean contains(IFeature search);
...
class MtLoFeature implements ILoFeature {
  ...
  /*
   * Fields: N/A Methods: - totalCapacity() - int - totalRating() - double -
  ...
  public int totalCapacityAcc(ILoFeature acc) {
    return 0;
  }

  public boolean contains(IFeature search) {
    return false;
  }
}
...
class ConsLoFeature implements ILoFeature {
   ...
   * restaurantInfo() - String Methods on Fields: - this.first.totalCapacity() -
   * int - this.first.totalRating() - double - this.first.restaurantCount() -
   * double - this.first.restaurantInfo() - String - this.rest.totalCapacity() -
  ...
  public int totalCapacity() {
    return this.totalCapacityAcc(new MtLoFeature());
  }

  public int totalCapacityAcc(ILoFeature acc) {
    if (acc.contains(this.first)) {
      return this.rest.totalCapacityAcc(acc);
    }
    else {
      return this.first.totalCapacityFeature(acc)
          + this.rest.totalCapacityAcc(new ConsLoFeature(this.first, acc));
    }
  ...
  public boolean contains(IFeature search) {
    return (this.first.isTheSame(search) || (this.rest.contains(search)));
  }
}
...
class Place {
   ...
   * Fields: - this.features.totalCapacity() - int - this.features.totalRating() -
  ...
  int totalCapacityAcc(ILoFeature acc) {
    return this.features.totalCapacityAcc(acc);
  }
...
interface IFeature {
  ...
  int totalCapacityFeature(ILoFeature acc); // This method gets the capacity of each feature

  boolean isTheSame(IFeature search);
...
class Restaurant implements IFeature {
  ...
  public int totalCapacityFeature(ILoFeature acc) {
    return 0;
  }
  ...
  public boolean isTheSame(IFeature search) {
    return search.sameName(this.name);
  }
...
class ShuttleBus implements IFeature {
  ...
  public int totalCapacityFeature(ILoFeature acc) {
    return this.destination.totalCapacityAcc(acc);
  }
...
class ExamplesPlaces {
  ...
  boolean testTotalCapacity(Tester t) {
    System.out.println(loganAirport.totalCapacityAcc(empty));
    return t.checkExpect(loganAirport.totalCapacity(), 150000 + 19580 + 30323);
  }

  boolean testContains(Tester t) {
    return t.checkExpect(neFeatures.contains(mikesPastry), true)
        && t.checkExpect(neFeatures.contains(tdGarden), true)
        && t.checkExpect(neFeatures.contains(legalSeaFoods), false)
        && t.checkExpect(empty.contains(tdGarden), false)
        && t.checkExpect(loganFeatures.contains(airportShuttle), true);

  }

  boolean testSameName(Tester t) {
    return t.checkExpect(auntieAnnes.sameName("Auntie Anne's"), true);
  }
}